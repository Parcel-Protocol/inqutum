 ...
 *
 * `frontend/lib/memo-compare.js` mirrors this file line for line and both are
...
const MEMO_TYPES: readonly string[] = ['none', 'text', 'id', 'hash', 'return'];
...
export function memosMatch(expected: MemoValue, actual: MemoValue): boolean {
  if (!isMemoType(expected?.type) || !isMemoType(actual?.type)) return false;
 ...
 */
export function horizonMemo(transaction: { memo?: string | null; memo_type?: string | null } | null | undefined): MemoValue {
  return { type: transaction?.memo_type ?? null, value: transaction?.memo ?? null };
}

export default { isMemoType, parseMemoId, decodeMemoHash, memosMatch, horizonMemo };
